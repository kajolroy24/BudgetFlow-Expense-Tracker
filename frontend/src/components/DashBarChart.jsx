...
const DashBarChart = () => {
  ...
  const CustomTooltip = ({ active, payload, label, darkMode }) => {
    if (active && payload && payload.length) {
      return (
        <div className='rounded-xl bg-[#faf5ff] dark:bg-[#352D49] border border-gray-100'
          // style={{
          //   borderRadius: '10px',
          //   border: darkMode ? 'none' : '1px solid #ccc',
          //   backgroundColor: darkMode ? '#352D49' : '#faf5ff',
          //   padding: '10px',
          // }}
        >
          <h4 className='dark:text-white'>{label}</h4> 
          <p className='text-[#7748ee]'>
            Expense: {payload[0].value}
          </p>
          <p className='text-[#43d9e2]'>
  ...
  return (
    <div className=' bg-white w-full border border-gray-200 rounded-2xl p-5 dark:bg-[#40375C] dark:border-none capitalize'>
      <h2 className='font-semibold text-xl mb-4 text-gray-700 dark:text-white'>Spending Analysis</h2>
      <ResponsiveContainer width={'100%'} height={320}>
        <BarChart data={data} barSize={30} >
          <CartesianGrid strokeDasharray="5 5" stroke={darkMode ? 'white' : '#E0E0E0'} vertical={false} />
          <XAxis dataKey="name" stroke={darkMode ? '#B5B0C7' : '#818181'} />
          <YAxis stroke={darkMode ? '#B5B0C7' : '#818181'} />
          {/* <Tooltip contentStyle={{ borderRadius: '10px', border: darkMode ? 'none' : '1px solid #ccc', backgroundColor: darkMode ? '#352D49' : '#faf5ff' }} cursor={{fill: darkMode ? '#b5b0c74a' : '#f3f4f6'}} /> */}
          <Tooltip 
        ...
        />
          <Legend
            content={({ payload }) => ( 
              <div className="flex flex-wrap justify-center space-x-4 mt-3 bg-gray-100 dark:bg-[#352D49] p-2 rounded-2xl">
