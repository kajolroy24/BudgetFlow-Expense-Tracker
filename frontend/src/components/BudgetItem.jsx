...
const BudgetItem = ({ budget, expense }) => {
    ...
    return (
        <div onClick={() => { navigate(`/dashboard/expenses/${budget._id}`); scrollTo(0, 0) }} className='p-5 bg-white border border-gray-200 dark:bg-[#211641] dark:border-none rounded-2xl hover:shadow-md cursor-pointer h-[160px]'>
            <div className='flex gap-2 items-center justify-between'>
                <div className='flex gap-2 items-center'>
                    <h2 className='text-2xl p-3 px-3 bg-slate-100 rounded-full dark:bg-[#140a2c]'>{budget.emoji}</h2>
                    <div>
                        <h2 className='font-bold capitalize dark:text-white'>{budget.name}</h2>
